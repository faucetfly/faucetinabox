    ...
    public function send($to, $amount, $userip, $referral = "false") {
        ...
        if (is_array($r) && array_key_exists("status", $r) && $r["status"] == 200) {
            return array(
                ...
                'html' => '<div class="alert alert-success">' . htmlspecialchars($amount) . ' satoshi was sent to you <a target="_blank" href="https://faucetfly.com/check/' . rawurlencode($to) . '">on FaucetFly.com</a>.</div>',
                'html_coin' => '<div class="alert alert-success">' . htmlspecialchars(rtrim(rtrim(sprintf("%.8f", $amount/100000000), '0'), '.')) . ' '.$this->currency.' was sent to you <a target="_blank" href="https://faucetfly.com/check/' . rawurlencode($to) . '">on FaucetFly.com</a>.</div>',
                'balance' => $r["balance"],
